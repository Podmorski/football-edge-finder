...
def test_gaps_flag_only_the_no_mozzart_leagues(tmp_path, monkeypatch):
    ...
    coverage.write({date(2026, 9, 26)}, {
        "eng_league_two": {"ps3838": 10, "mozzart": 0},   # gap
        "esp_segunda": {"ps3838": 10, "mozzart": 6},      # Mozzart is there
        "league_one_t3": {"ps3838": 4, "mozzart": 0},     # below the >= 5 threshold
        "ger_liga3": {"ps3838": 1, "mozzart": 0},         # below the threshold
    })
...
def test_a_stale_record_is_not_trusted(tmp_path, monkeypatch):
    ...
    coverage.write({date(2026, 9, 26)}, {"eng_league_two": {"ps3838": 10, "mozzart": 0}},
                   when=old)
...
def test_health_puts_the_gap_banner_at_the_top(tmp_path, monkeypatch):
    monkeypatch.setattr(coverage, "PATH", tmp_path / "coverage.json")
    coverage.write({date(2026, 9, 26)}, {
        "esp_segunda": {"ps3838": 10, "mozzart": 0},
        "eng_premier": {"ps3838": 8, "mozzart": 8},
    })
